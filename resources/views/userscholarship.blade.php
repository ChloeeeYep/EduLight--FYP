...
<body id="top">
  ...
  <!--- #HEADER-->
    ...
    <header class="header" >
      ...
      <main>
        <article>
        <div class="scholarship-heading-container" style="text-align: center">
          <h1>
            Scholarship
          </h1>
        </div>
          ...
          <section class="section course" id="courses" aria-label="course">
             <div class="container" style="max-width: 1400px">
              <div class="courses-header">
                <b><p class="section-subtitle" style="color: black;">We found <span class="color" style="color: #0056d2;">{{ $totalScholarships }}</span> Scholarship available for you</p></b>

                 <!-- Fund Type -->
                <div class="dropdown">
                    <button class="dropbtn">
                        <i class='bx bx-filter' style="font-size:25px"></i>
                        {{$currentFund}} <i class="fa fa-chevron-down" aria-hidden="true" style="margin-left:41px"></i>
                    </button>
                    <div class="dropdown-content">
                      <a href="{{ url('/filteruserscholarship?fundType=Scholarship&academic=' . Request::get('academic') . '&status=' . Request::get('status')) }}">Scholarship</a>
                      <a href="{{ url('/filteruserscholarship?fundType=Education&academic=' . Request::get('academic') . '&status=' . Request::get('status')) }}">Education</a>
                      <a href="{{ url('/filteruserscholarship?fundType=Loan&academic=' . Request::get('academic') . '&status=' . Request::get('status')) }}">Loan</a>
                    </div>
                </div>
                ...
                <div class="dropdown">
                    <button class="dropbtn">
                        <i class='bx bx-filter' style="font-size:25px"></i>
                        {{$currentAcademic}} <i class="fa fa-chevron-down" aria-hidden="true" style="margin-left:41px"></i>
                    </button>
                    <div class="dropdown-content">
                      <a href="{{ url('/filteruserscholarship?academic=All&fundType=' . Request::get('fundType') . '&status=' . Request::get('status')) }}">All</a>
                      <a href="{{ url('/filteruserscholarship?academic=Foundation&fundType=' . Request::get('fundType') . '&status=' . Request::get('status')) }}">Foundation</a>
                      <a href="{{ url('/filteruserscholarship?academic=Diploma&fundType=' . Request::get('fundType') . '&status=' . Request::get('status')) }}">Diploma</a>
                      <a href="{{ url('/filteruserscholarship?academic=Degree&fundType=' . Request::get('fundType') . '&status=' . Request::get('status')) }}">Degree</a>
                    </div>
                </div>
                ...
                <div class="dropdown">
                    <button class="dropbtn">
                        <i class='bx bx-filter' style="font-size:25px"></i>
                        {{$currentStatus}} <i class="fa fa-chevron-down" aria-hidden="true" style="margin-left:41px"></i>
                    </button>
                    <div class="dropdown-content">
                      <a href="{{ url('/filteruserscholarship?status=Coming&fundType=' . Request::get('fundType') . '&academic=' . Request::get('academic')) }}">Coming</a>
                      <a href="{{ url('/filteruserscholarship?status=Ongoing&fundType=' . Request::get('fundType') . '&academic=' . Request::get('academic')) }}">Ongoing</a>
                      <a href="{{ url('/filteruserscholarship?status=End&fundType=' . Request::get('fundType') . '&academic=' . Request::get('academic')) }}">End</a>
                    </div>
                </div>

                <a href="{{ route('userscholarship') }}" class="btn-reset">
                ...
                <div class="scholarship-container">
                  <a href="{{route('scholarshipexplaine')}}" class="scholarship-explanation-link">*Explain of Scholarships</a>
                    <table class="scholarship-table">
                        <thead>
                            <tr>
                                <th style="width: 20%;">Univeristy</th>
                        ...
                        @if (count($scholarships) > 0)
                            @foreach ($scholarships as $scholarship)
                                <tr>
                                    <td style="text-align: center;">{{ $scholarship->user->username ?? 'N/A' }}</td>
                                    <td style="text-align: center;">                            
                                        <a href="{{route('userscholarshipdetail', $scholarship->id)}}" style="color: red; text-decoration: none;">
                                          {{ $scholarship->title }}                             
                                        </a>
                                    </td>
                                    <td class="ellipsis">{{ $scholarship->description }}</td>
                                    <td style="text-align: center;">
                                        <span class="status {{ strtolower($scholarship->status) }}">
                                            <i class="fas fa-clock"></i> {{ $scholarship->status }}
                                        </span>
                                    </td>
                                </tr>
                            @endforeach
                            @else
                              <tr>
                                <td colspan="4" style="text-align: center;">No scholarship found</td>
                              </tr>
                            @endif
                        </tbody>                 
                    </table>
