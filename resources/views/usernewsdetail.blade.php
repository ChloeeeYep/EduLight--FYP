...
<head>
  ...
  <!--- custom css link-->
  <link rel="stylesheet" href="{{ URL::asset('asset/css/usernewsdetails.css') }}">
            ...
            <div class="container">
                <div class="row row--30">
                    <div class="col-lg-8">
                        ...
                        <a href="{{route('usernews')}}" class="back-button">
                          <ion-icon name="arrow-back-outline"></ion-icon>
                            ...
                            <div class="entry-content">
                                ...
                                <li class="card-meta-item">
                                  <ion-icon name="calendar-outline" aria-hidden="true" class= "calender"></ion-icon>
                                  <span class="span">{{ $item->created_at->format('Y-m-d') }}</span>
                                </li>
                                <div class="thumbnail">
                                  <img src="/images/{{ $item->image }}"  alt="Blog Image">
                                </div>
                            </div>
                            <textarea class="description" readonly disabled>{{ $item->description }}</textarea>
                            @if(!is_null($item->title1) && $item->title1 !== '')
                              <h3 class="title1">{{ $item->title1 }}</h3>
                          @endif
                          @if(!is_null($item->description1) && $item->description1 !== '')
                              <textarea class="description" readonly disabled>{{ $item->description1 }}</textarea>
                          @endif
                        </div>                                                                                                                                                   
                    </div>
                    <div class="col-lg-4">
                      <div class="edu-blog-sidebar">                          
                            ...
                            <div class="latest-post">
                                <a href="{{ route('usernewsdetail' , $news->id) }}" class="latest-post-link">
                                  <img src="{{ asset('images/'.$news->image) }}"  alt="Blog Images" class="thumbnailsmall">
                                    <div class="post-info">
                                        <h6 class="title">{{ $news->title }}</h6>
                                        <div class="meta-info"> <!-- New flex container for the icon and date -->
                                          <ion-icon name="calendar-outline" aria-hidden="true" class="calender1"></ion-icon>
