...
<head>
	...
	<title>Instructor | EduLight</title>
</head>
<body>
	...
	<section id="content">
		...
		<main>
			<div class="head-title">
                ...
                <form action="searchinstructor" method="GET">
				...
				<a href="{{ route('instructor') }}" class="btn-reset">
					<i class='bx bx-refresh'></i>
					<span class="text">Reset</span>
				</a>

			    <a href="{{route('createinstructor')}}" class="btn-add">
                    ...
                    <table>
						...
						@php $counter = 1; @endphp
                            @foreach($instructors as $instructor)
                                <tr>
                                    <td>{{ $counter++ }}</td>
                                    <td><img src="/images/{{ $instructor->image }}" style="width:110px;height:120px;"></td>
									<td>{{ $instructor->name }}</td>
                                    <td>
                                        <div class="button-group">
                                            <a href="{{ route('showinstructor' , $instructor->id) }}" class="btn-action btn-info">
                                                <i class='bx bx-bullseye'></i>
                                                <span>View</span>
                                            </a>
                                            <a href="{{ route('editinstructor' , $instructor->id) }}" class="btn-action btn-edit">
                                                <i class='bx bx-edit-alt'></i>
                                                <span>Edit</span>
                                            </a>
                                            <form action ="{{ route('destroyinstructor' , $instructor->id) }}" method="POST" class="form-action">
												@csrf
                                                @method('DELETE')
